...
if [ ! -x $curator ] ; then echo 'curator not found'; exit 1; fi
touch $log
...
$curator --host $host close --older-than 33 --prefix host_log_ --timestring %Y%m%d 2>$log || (echo "failed close" && exit 1)
